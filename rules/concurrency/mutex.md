...
impl Context {
    ...
    // None while active; otherwise the immutable first committed terminal cause.
...
§ 15(8) Context has one coherent first-wins terminal transition from active to
either `ContextError.Cancelled` or `ContextError.DeadlineExceeded`. Once
committed, later cancellation/deadline events cannot replace `Context.Error`.
`IsCancelled` and `Error` observe the same published terminal state.
