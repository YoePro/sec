...
| `foundations/operators.md` | **Written** | Canonical operator semantics; dynamic-array membership is accepted by Sema, with nominal element compatibility retained. Dynamic-array backend support remains pending in `governance/sema.yaml`; older operator work is tracked in the legacy ledger. |
