...
| `compiler/parser_recovery.md` | **Written** | Canonical deterministic recovery model; ordinary unterminated statement blocks, switch/select bodies, match arms, and try handlers retain partial AST contents for tooling while errors still block code generation. Invalid type metadata covers base and postfix families and now propagates diagnosed callable, sequence, array-length, and shaped-constant children to their composite outer types. Select header recovery guarantees progress; expression-owned generic arguments and other specialized recovery remain partial. |
