...
**§ 1(8)** `rules/memory/references.txt` and `rules/memory/lifetime_analysis.txt` retain their specialized responsibilities until replaced by their revision-2 successors. This rulebook must not invent visible lifetime parameters or pre-empt future physical-addressing rules.
