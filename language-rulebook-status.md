...
| `foundations/grammar.md` | **Written** | Canonical grammar distinguishes instance let from explicitly static impl bindings. Interpolation holes have frontend formatting-contract selection and mixed concat/interpolation chains have maximal frontend plans; allocation/IR/backend completion remains pending. |
| `foundations/operators.md` | **Written** | Canonical operator semantics; `in` and contextual `not in` membership are accepted by the frontend with nominal element compatibility retained. Statement-only `++` and `--` are implemented as parser-normalized compound assignments with formatter and LSP support in `frontend.increment-decrement-aliases`. Interpolation formatting contracts resolve to immutable Sema facts and mixed finite chains resolve to one maximal `StringConcatPlan`; allocation policy, Semantic IR consumption, and lowering remain pending. Dynamic-array backend support remains pending in `governance/sema.yaml`; logical short-circuit flow is tracked by `frontend.logical-short-circuit-flow` in `governance/control_flow.yaml`, while other older operator work remains in the legacy ledger. |
...
| `control-flow/flowcontrol_while.md` | **Written** | Canonical revision 2.0 condition-controlled loops, boolean conditions, loop-control targets, non-continuing loops, flow merging, and explicit Sec 0.1 exclusions. Implementation progress is tracked by `frontend.while-statements-v2` and `frontend.logical-short-circuit-flow` in `governance/control_flow.yaml`. |
...
| `control-flow/flowcontrol_match.md` | **Written** | Canonical revision 2.0 structural and variant matching, exhaustiveness, guarded ownership commit, contextual arm-block values, union empty state, and match/LSP facts. Implementation progress is tracked by `frontend.match-v2` and `frontend.match-termination-consistency` in `governance/control_flow.yaml`. |
...
| Rulebook | Status | Notes |
|---|---|---|
| `collections/collections.md` | **Written** | Canonical fixed-array, owning dynamic-array, slice, list, map, and set semantics. Frontend progress is tracked by `frontend.collections` in `governance/collections.yaml`. |
| `collections/shaped-types.md` | **Written** | Canonical runtime/static shaped values, affine views, layout, storage requests and transfer, broadcasting, vector/matrix algebra, and contraction semantics. Implementation progress is tracked by `frontend.shaped-types` in `governance/collections.yaml`. |
...
| Rulebook | Status | Notes |
|---|---|---|
| `errors/errorhandling.md` | **Written** | Canonical revision 2.1 compiler-known `error`, typed Result channels, direct compatible Result/Option carrier returns, Result projections, general Result/Option/fallible-operation `try`, partial guarded handlers, explicit fallible-setter contracts, ownership, Semantic IR, diagnostics, and LSP requirements. Parser rejects obsolete nested `match` try wrappers with P2012; Sema reports shadowed Err handlers with S1045 and rejects non-discardable concrete Err(_) payloads in try and match with S1006. Open-error discardability and broader handler semantics remain partial. Implementation progress is tracked by `frontend.errorhandling-v2` in `governance/errors.yaml`. |
| `errors/panic.md` | **Written — revision 2.2** | Canonical panic domains, containment, cleanup, checked unreachable, task/thread/process outcomes, exact `PanicID uint32` and five-field `PanicInfo`, no-panic verification, and runtime-free support model. Sec 0.1 assertion syntax is locked to `assert condition` or `assert condition, "message"`; frontend effect analysis records unproven assertions as `MayPanic`, proves literal `true` panic-free, and treats literal `false` as non-returning, while broader proof and lowering remain. Explicit panic is locked to the statement `panic "message"`. Exact build-manifest syntax remains open. |
...
| `compiler/compiler_known_members.md` | **Written** | Canonical typed registry, stable member identities, fallback-versus-authoritative policy, universal `ToString() string`, property-only `SizeOf`, complete compiler-known declaration governance, core boundary, and tooling behavior. Sema/LSP inherit eligible trusted-core string methods and properties for named strings; broader registry integration remains partial. |
...
| `analysis/pitfall_analysis.md` | **Written** | Canonical semantic pitfall-finding, evidence, suppression, confidence, corrective-action, budget, incremental, LSP, and FFI-contract model. `sema.pitfall-analysis` is tracked in `governance/analysis.yaml`; endpoint suppression recognizes matched then-branch and else-branch break, continue, or return guards and only applies after the guard. |
...
| `compiler/parser_recovery.md` | **Written** | Canonical deterministic recovery model; ordinary unterminated statement blocks, switch/select bodies, match arms, and try handlers retain partial AST contents for tooling while errors still block code generation. Invalid type metadata covers base and postfix families, propagates diagnosed callable, sequence, array-length, and shaped-constant children, and now retains empty function/lambda parameter type positions without losing later parameters or declarations. Select header recovery guarantees progress; expression-owned generic arguments and other specialized recovery remain partial. |
