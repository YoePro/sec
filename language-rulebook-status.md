...
| `analysis/pitfall_analysis.md` | **Written** | Canonical semantic pitfall-finding, evidence, suppression, confidence, corrective-action, budget, incremental, LSP, and FFI-contract model. `sema.pitfall-analysis` is tracked in `governance/analysis.yaml`; endpoint suppression recognizes matched then-branch and else-branch exit guards and only applies after the guard. |
